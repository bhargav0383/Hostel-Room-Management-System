...
const MyPod = ({ id }) => {
  ...
  return (
    <div className="container">
      ...
      <h2 className='text-center'>Pod Mates</h2>
      <div className="row">
      ...
      <h3 className='text-center'>{block} Floor-{floor} Facilities</h3>
      <div className="row">
        ...
        <div className="col-md-3">
          <div className="border rounded p-3">
            <div className="d-flex justify-content-between align-items-center">
              <h5 className="mb-0">Water Filter</h5>
              <span className={`badge bg-${floorData.water_filter_working_status ? 'success' : 'danger'}`}>
                {floorData.water_filter_working_status ? 'Working' : 'Not Working'}
              </span>
            </div>
            <div className="mt-3">
              <p className="timestamp-text">Last serviced at {new Date(floorData.water_filter_last_serviced).toLocaleString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: true, timeZone: 'Asia/Kolkata' })}</p>
            </div>
          </div>
        </div>
        <div className="col-md-3">
